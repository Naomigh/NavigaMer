...
void MetricTree::insert(SequenceId sequence_id, std::uint32_t payload) {
  ...
  auto prepared = distance_.prepare(sequences_.sequence(sequence_id));
  std::uint32_t current = 0;
  while (true) {
    const int d = prepared(sequences_.sequence(nodes_[current].sequence_id));
...
bool MetricTree::any_within(SequenceId query, std::uint32_t radius,
  ...
  auto prepared = distance_.prepare(sequences_.sequence(query));
  std::vector<std::uint32_t> stack{0};
  while (!stack.empty()) {
    const auto node_id = stack.back();
    stack.pop_back();
    const int d = prepared(sequences_.sequence(nodes_[node_id].sequence_id));
